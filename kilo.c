...
void editorFindCallBack(char* query, int key) {
  ...
  for (i = 0; i < E.numrows; i++) {
    current += direction;
    if (current == -1)
      // reverse search and reached the top, go to the end
      current = E.numrows - 1;
    else if (current == E.numrows)
      // forward search and reached the bottom, back to the top
      current = 0;
    ...
    if (match) {
      ...
      E.cx = editorRowRxToCx(
          row, match - row->render);  // match's addr - row beginning's addr
      E.rowoff = E.numrows;

      saved_hl_line = current;
      saved_hl = malloc(row->rsize);
      memcpy(saved_hl, row->hl, row->rsize);
      memset(&row->hl[match - row->render], HL_MATCH, strlen(match));
      break;
    }
  }
}
...
void editorFind() {
  ...
  if (query) {
    ...
    // find aborted, cursor back to before
    E.cx = saved_cx;
    E.cy = saved_cy;
...
// update vision window's offset and 'render field x'
void editorScroll() {
  E.rx = 0;
  if (E.cy < E.numrows) {
...
void editorRefreshScreen() {
  ...
  editorDrawRows(&ab);  // draw the contents
  ...
  write(STDOUT_FILENO, ab.b, ab.len);  // refresh screen
  abFree(&ab);
}

void editorSetStatusMessage(const char* fmt, ...) {
  va_list ap;         // list of arguments
  va_start(ap, fmt);  // tell c the 'fmt' is before '...'
...
char* editorPrompt(char* prompt, void(callback)(char*, int)) {
  ...
  while (1) {
    ...
    if (c == DEL_KEY || c == CTRL_KEY('h') || c == BACKSPACE) {
      ...
      // press 'esc' to abort prompt
      editorSetStatusMessage("");
      if (callback) callback(buf, c);
      free(buf);
      return NULL;
    } else if (c == '\r') {
      // press 'enter' to pass through the string
      if (buflen != 0) {
        editorSetStatusMessage("");
        if (callback) callback(buf, c);
        return buf;
